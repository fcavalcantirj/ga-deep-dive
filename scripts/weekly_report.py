...
def generate_report(property_name, current_data, changes, top_pages, top_sources):
    ...
    report.append("")
    report.append("---")
    report.append(f"*Generated by Claudius 🏴‍☠️ at {datetime.now().strftime('%Y-%m-%d %H:%M')} UTC*")
...
def send_email(subject, body, to_email):
    ...
    try:
        ...
        if not api_key:
            print("AgentMail API key not found")
            return False
...
def main():
    ...
    send_email(subject, full_report, 'felipe.cavalcanti.rj@gmail.com')
